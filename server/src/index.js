import "./config.js"; // Isko pehele hi rkhna pdega
...
// Custom Clerk Auth Middleware (Manual Bypass h 401 ko prevent krne kr liye)
const clerkAuthMiddleware = (req, res, next) => {
  ...
  if (authHeader && authHeader.startsWith("Bearer ")) {
    try {
      ...
      console.warn(
        "[AUTH] Could not parse token, continuing as guest:",
        err.message,
      );
...
app.use("/api/admin", clerkAuthMiddleware, adminRoutes);
app.use("/api/payments", clerkAuthMiddleware, paymentRoutes);
