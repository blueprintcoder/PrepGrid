...
/* Pura logic jo interview response dekhega */ 
...
export const processInterviewTurn = async (sessionId, userMessage) => {
  ...
  // 1. current answer ko evaluate krega.
  const currentLevel = session.currentLevel; // current level dekhega (easy, medium, hard)
  const qIndex = session.currentQuestionIndex; // initially ye 0 h.
  ...
  // 2. User ke answer ko DB me save krega aur AI ki rating or correction bhi.
  ...
  if (isLastQuestionInLevel) {
    if (!passesLevel) {
      ...
      // Baadme yha XState se manage krna h naki if-else se...bs testing ke liye krna h if-else;
  ...
  if (triggerAudit) {
    try {
      ...
      // Data ko vectorDB me store kr rha hu mai (Background task h ye) .catch() bhi rkh rha hu taki koi error aaye to fail na ho.
    ...
    // Select next questiom from the question bank.
