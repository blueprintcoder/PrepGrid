...
import SessionHistory from './features/results-dashboard/SessionHistory';
import BillingView from './features/results-dashboard/BillingView';
import Logo from './components/Logo';
import { SignedIn, SignedOut, SignInButton, SignUpButton, UserButton } from '@clerk/clerk-react';
import { Github, Linkedin, Mail, MessageSquare, Shield, Zap, Target, ArrowRight, History, CreditCard } from 'lucide-react';

function App() {
  const [step, setStep] = useState('upload'); // upload, interview, results, history
  ...
  const handleViewHistorySession = (id) => {
    setSessionId(id);
    setStep('results');
  };
          ...
          <div className="flex items-center gap-6">
            ...
            <SignedIn>
              <button 
                ...
                className={`flex items-center gap-2 text-sm font-bold transition-colors ${step === 'history' ? 'text-[#14532D]' : 'text-[#4B5563] hover:text-[#14532D]'}`}
              >
                <History className="w-4 h-4" /> History
              </button>
              <button 
                onClick={() => setStep('billing')}
                className={`flex items-center gap-2 text-sm font-bold transition-colors ${step === 'billing' ? 'text-[#14532D]' : 'text-[#4B5563] hover:text-[#14532D]'}`}
              >
            ...
            <div className="relative">
              ...
              {step === 'history' && (
                <SessionHistory 
                  ...
                  onViewResult={handleViewHistorySession} 
                />
              )}
              {step === 'billing' && (
                <BillingView 
                  onBack={() => setStep('upload')} 
                  onUpgradeSuccess={() => setStep('upload')} 
                />
              )}
              {step === 'interview' && (
                <InterviewRoom 
