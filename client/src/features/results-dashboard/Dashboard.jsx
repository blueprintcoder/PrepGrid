...
import { 
  ...
  History, TrendingUp, Calendar, ChevronRight, Award, Flame, Zap, Target
} from 'lucide-react';
...
const Dashboard = ({ sessionId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch user metadata (streak, etc.)
        const userRes = await axios.get(`${API_BASE_URL}/api/interview/user-profile`, { headers });
  ...
  const radarData = [
    ...solidAreas.slice(0, 3).map(skill => ({ subject: skill, A: 90, fullMark: 100 })),
    ...areasToImprove.slice(0, 2).map(skill => ({ subject: skill, A: 60, fullMark: 100 })),
    ...areasToLearn.slice(0, 2).map(skill => ({ subject: skill, A: 30, fullMark: 100 })),
  ];

  if (radarData.length < 3) {
    const placeholders = ['System Design', 'Algorithms', 'Logic'];
    while(radarData.length < 3) {
      radarData.push({ subject: placeholders[radarData.length], A: 50, fullMark: 100 });
    }
  }
    ...
    <div className="p-4 lg:p-8 space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-4 bg-white p-2 rounded-2xl border border-[#E5E7EB] shadow-sm">
          <div className="px-4 py-2 flex items-center gap-2 border-r border-[#E5E7EB]">
            <Flame className="w-5 h-5 text-orange-500 fill-orange-500" />
            <div className="flex flex-col">
              <span className="text-xs font-bold text-[#111827]">{userData?.streak || 0}</span>
              <span className="text-[8px] font-bold text-[#9CA3AF] uppercase">Streak</span>
            </div>
          </div>
          <div className="px-4 py-2 flex items-center gap-2 border-r border-[#E5E7EB]">
            <CheckCircle2 className="w-5 h-5 text-[#14532D]" />
            <div className="flex flex-col">
              <span className="text-xs font-bold text-[#111827]">{userData?.solvedQuestions?.length || 0}</span>
              <span className="text-[8px] font-bold text-[#9CA3AF] uppercase">Solved</span>
            </div>
          </div>
          <div className="px-4 py-2 flex items-center gap-2 pr-4">
            <Target className="w-5 h-5 text-[#65A30D]" />
            <div className="flex flex-col">
              <span className="text-xs font-bold text-[#111827]">{history.length}</span>
              <span className="text-[8px] font-bold text-[#9CA3AF] uppercase">Mocked</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[32px] border border-[#E5E7EB] shadow-sm">
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="text-[10px] font-bold text-[#6B7280] uppercase tracking-widest">Skill Radar</span>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8 items-start">
        {/* Session Archive */}
        <div className="lg:col-span-3 bg-white p-8 rounded-[32px] border border-[#E5E7EB] shadow-sm h-full max-h-[500px] flex flex-col">
